...
class CakeMain extends React.Component {
    state = {
    ...
    // 请流数据的函数，为datalist中新增六条数据
    async requestdata(){
        const{data: newdatalist} = await request.get('good/allcakes', {
            params: {
                page: this.state.initialIndex,
                maxcount: 6
            }
        })
        console.log("cakemain.newdatalist",newdatalist.data);
        let arr=this.state.datalist;
        arr.push.apply(arr,newdatalist.data)
        console.log("cakemain.arr",arr);
        this.setState({
            datalist: arr,
